    ...
    <div className="min-h-screen bg-background text-foreground p-6 flex flex-col gap-4 select-none">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
        ...
        {viewMode === "diario" ? (
          ...
          <KPICard title="Média ating. meta" value={`${monthlyMetrics.mediaAtingimentoMeta.toFixed(1)}%`} rawValue={monthlyMetrics.mediaAtingimentoMeta} valueSuffix="%" valueDecimals={1} subtitle={new Date(selectedYear, selectedMonth - 1).toLocaleString("pt-BR", { month: "long", year: "numeric" }).replace(/^\w/, c => c.toUpperCase())} icon={<Calculator className="w-5 h-5" />} />
        )}
